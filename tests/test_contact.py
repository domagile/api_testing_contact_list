...
def test_add_contact(base_url, contact_data_1, register_user):
    ...
    response = create_contact(base_url, contact_data_1, token)
    actual_user_data = response.json()
    check_contact_data(actual_user_data, contact_data_1, user_data.get("_id"), response.text)
...
def test_get_contact_list(base_url, contact_data_1, contact_data_2, contact_data_3, register_user):
    ...
    response = create_contact(base_url, contact_data_2, token)
    expected_contact_2_id = response.json().get("_id")
    print(f"Expected contact 2 id: {expected_contact_2_id}")

    response = create_contact(base_url, contact_data_3, token)
    ...
    check_contact_data(actual_contact_1, contact_data_1, user_id, contact_list_response.text)
    ...
    check_contact_data(actual_contact_2, contact_data_2, user_id, contact_list_response.text)
    ...
    check_contact_data(actual_contact_3, contact_data_3, user_id, contact_list_response.text)
...
def test_get_contact(base_url, contact_data_1, contact_data_2, contact_data_3, register_user):
    ...
    check_contact_data(actual_contact_2, contact_data_2, user_data.get("_id"), response.text)
...
def test_update_contact_put(base_url, contact_data_1, contact_data_2, register_user):
    ...
    check_contact_data(actual_contact_data, contact_data_2, user_data.get("_id"), response.text)
...
def create_contact(base_url, contact_data, token):
    response = requests.post(f"{base_url}/contacts", json=contact_data, headers=create_headers(token))
    assert response.status_code == HTTPStatus.CREATED, f"Error [add contact]: {response.status_code}"
    return response

def check_contact_data(actual_data, expected_data, user_id, response_text):
    assert "_id" in actual_data, f"Missing id: {response_text}"
    ...
    assert actual_data["owner"] == user_id, "Error: owner does not match"
    assert "__v" in actual_data, f"Missing version: {response_text}"